...
int GDSParse_ogl::SetTopcell(const char *topcell)
{
	...
	// Find new topcell
	if(topcell)
		_topcell = (GDSObject_ogl*) _Objects->SearchObject(topcell);
    if(!_topcell)
	{
		if(topcell)
...
void GDSParse_ogl::buildSubstrate()
{
    ...
    if(layer && !substrate)
    {
        ...
        int tp =
        renderer.addVertex(bounds.min.X-(bounds.max.X-bounds.min.X)*0.05f, bounds.min.Y-(bounds.max.Y-bounds.min.Y)*0.05f, (layer->Height+layer->Thickness)*_units);
        renderer.addVertex(bounds.max.X+(bounds.max.X-bounds.min.X)*0.05f, bounds.min.Y-(bounds.max.Y-bounds.min.Y)*0.05f, (layer->Height+layer->Thickness)*_units);
        renderer.addVertex(bounds.max.X+(bounds.max.X-bounds.min.X)*0.05f, bounds.max.Y+(bounds.max.Y-bounds.min.Y)*0.05f, (layer->Height+layer->Thickness)*_units);
        renderer.addVertex(bounds.min.X-(bounds.max.X-bounds.min.X)*0.05f, bounds.max.Y+(bounds.max.Y-bounds.min.Y)*0.05f, (layer->Height+layer->Thickness)*_units);
        
        int bp = 
        renderer.addVertex(bounds.min.X-(bounds.max.X-bounds.min.X)*0.05f, bounds.min.Y-(bounds.max.Y-bounds.min.Y)*0.05f, layer->Height*_units);
        renderer.addVertex(bounds.max.X+(bounds.max.X-bounds.min.X)*0.05f, bounds.min.Y-(bounds.max.Y-bounds.min.Y)*0.05f, layer->Height*_units);
        renderer.addVertex(bounds.max.X+(bounds.max.X-bounds.min.X)*0.05f, bounds.max.Y+(bounds.max.Y-bounds.min.Y)*0.05f, layer->Height*_units);
        renderer.addVertex(bounds.min.X-(bounds.max.X-bounds.min.X)*0.05f, bounds.max.Y+(bounds.max.Y-bounds.min.Y)*0.05f, layer->Height*_units);
